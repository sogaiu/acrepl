...
(defun acrepl-send-ascertained-region ()
  ...
  (interactive)
  (condition-case err
      (cl-destructuring-bind (start end) (ab-sexp-bounds (list 'def
                                                               'defn
                                                               'defn-
                                                               'defmacro
                                                               'ns
...
(defun acrepl-act-on-sexp-at-point (a-fn)
  "Call A-FN on detected sexp at (before) point."
  (let* ((thing (sp-get-thing t))
         (start (sp-get thing :beg))
         (end (sp-get thing :end))
         (prefix-l (sp-get thing :prefix-l)))
    (when (and start end)
      (funcall a-fn (- start prefix-l) end))))

(defun acrepl-send-expression-at-point ()
  "Send expression at point."
  (interactive)
  (acrepl-act-on-sexp-at-point #'acrepl-send-region))

(defun acrepl-tap-expression-at-point ()
  "Apply tap> to expression at point."
  (interactive)
  (acrepl-act-on-sexp-at-point #'acrepl-tap-region))
